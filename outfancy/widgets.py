...
def create_matrix_color_optimized(x, y, fill=''):
    
    # Expects colors in the following format: color1{}color2

    colors = ansi_escape.findall(fill)
    print(len(colors))
    opt_fill = remove_colors(fill)

    begin_fill = colors[0] + opt_fill
    end_fill = opt_fill + colors[1]
    ...
    for row in range(y):
        ...
        for col in range(x):
            if row == 0 and col == 0:
                mat[row].append(begin_fill)
            elif row == y-1 and col == x-1:
                mat[row].append(end_fill)
            else:
                mat[row].append(opt_fill)
    return mat
